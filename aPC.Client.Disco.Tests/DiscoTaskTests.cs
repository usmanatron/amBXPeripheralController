...
namespace aPC.Client.Disco.Tests
{
  ...
  internal class DiscoTaskTests
  {
    ...
    {
      var interval = (int)(1.5 * settings.PushInterval);
      var task = new Task(discoTask.Run);
      ...
      Assert.AreEqual(lightSectionGenerator.GeneratedScene(), notificationClient.CustomScenesPushed[0].Frames[0].LightSection);
    }
  }
}